...
//
// FORK (ameba-site): ameba-site is a plain-JS/JSX app with no .d.ts anywhere
// — findTypesRoot/projectFor always miss, so the upstream implementation
// returns an empty set here, which zeroes out every storybook-discovered
// component at the "must be a public export" filter in package-build.mjs
// (components = src.components.filter(c => exported.has(c.name))). Since
// there's no type-level export surface to check, fall back to a plain
// source scan of cfg.entry (the synthetic entry.js re-exporting the synced
// components — see .design-sync/config.json + .design-sync/entry.js) for
// PascalCase named exports, mirroring the same regex approach
// package-build.mjs already uses for cfg.extraEntries source scanning.
// Declared in cfg.libOverrides. See .design-sync/NOTES.md.
export function exportedNames(pkgDir, pkgJson) {
  ...
  if (names.size === 0) {
    for (const cand of ['.design-sync/entry.js', '.design-sync/entry.jsx']) {
      const p = join(pkgDir, cand);
      if (!existsSync(p)) continue;
      const text = readFileSync(p, 'utf8');
      for (const m of text.matchAll(/export\s*\{\s*default\s+as\s+([A-Za-z_$][\w$]*)\s*\}/g)) names.add(m[1]);
      for (const m of text.matchAll(/export\s+(?:async\s+)?(?:const|let|var|function|class)\s+([A-Za-z_$][\w$]*)/g)) names.add(m[1]);
      ...
      names.forEach((n) => { if (!/^[A-Z]/.test(n)) names.delete(n); });
      break;
    }
  }
  return names;
