...
import { gsap, SplitText, ScrollTrigger, DESKTOP_QUERY, prefersReducedMotion } from "../../utils/gsapSetup";
...
function Associacio() {
  ...
  useGsapContext(() => {
    ...
    const principisTexts = gsap.utils.toArray(".associacio__principis-cols p", root);
    ...
    const statLabels = statCols.map((c) => c.querySelector(".associacio__stat-label"));

    const grups = gsap.utils.toArray(".associacio__grup", root);
    const grupCircles = grups.map((g) => g.querySelector(".associacio__grup-circle"));

    const allTargets = [
      cta, ctaTitle, ...ctaBullets, ctaButton,
      ...blobCircles, ...blobTitles, ...blobParagraphs,
      sinceTitle, sinceBody,
      principisBand, ...principisNumbers, ...principisTexts,
      ...statCols, ...statLabels,
      ...grups, ...grupCircles,
    ].filter(Boolean);
    ...
    if (cta) {
      ...
      if (ctaButton) gsap.set(ctaButton, { y: 14, scale: 0.94, autoAlpha: 0 });
      gsap
        .timeline({ scrollTrigger: { trigger: cta, start: "top 85%", once: true } })
        .to(cta, { scaleX: 1, duration: 0.6, ease: "expo.out" })
        .to(ctaWords, { yPercent: 0, duration: 0.5, stagger: 0.05, ease: "power3.out" }, "-=0.35")
        .to(ctaBullets, { x: 0, autoAlpha: 1, duration: 0.4, stagger: 0.08 }, "-=0.3")
        .to(ctaButton, { y: 0, scale: 1, autoAlpha: 1, duration: 0.4 }, "-=0.2");
    }

    // §1.1 — blobs: circle scale+rotate, char-split title, paragraph.
    if (blobs.length) {
      gsap.set(blobCircles, { scale: 0, rotate: -12, transformOrigin: "center" });
      ...
      const tl = gsap.timeline({ scrollTrigger: { trigger: blobs[0], start: "top 80%", once: true } });
      blobs.forEach((_, i) => {
        const at = i * 0.12;
        tl.to(blobCircles[i], { scale: 1, rotate: 0, duration: 0.6, ease: "expo.out" }, at);
        if (blobCharsPerBlock[i].length) {
          tl.to(blobCharsPerBlock[i], { yPercent: 0, duration: 0.4, stagger: 0.03, ease: "power3.out" }, at + 0.25);
        }
        if (blobParagraphs[i]) {
          tl.to(blobParagraphs[i], { y: 0, autoAlpha: 1, duration: 0.4 }, at + 0.45);
        }
      });
    }
    ...
    if (sinceEl && sinceTitle) {
      gsap.matchMedia().add(DESKTOP_QUERY, () => {
        ...
        const tl = gsap.timeline({
          scrollTrigger: { trigger: sinceEl, start: "top 80%", end: "top 20%", scrub: 1 },
        });
        tl.to(lines, { yPercent: 0, stagger: 0.3, duration: 1, ease: "none" }, 0).to(
          sinceTitle,
          { webkitTextStrokeWidth: "1px", duration: 1 },
          0,
        );
        if (sinceBody) tl.to(sinceBody, { x: 0, autoAlpha: 1, duration: 0.6, ease: "none" }, 0.6);
      });
      ...
      gsap.matchMedia().add("(max-width: 759px)", () => {
        ...
        gsap
          .timeline({ scrollTrigger: { trigger: sinceEl, start: "top 75%", once: true } })
          .to(lines, { yPercent: 0, duration: 0.5, stagger: 0.1, ease: "power3.out" })
          .to(sinceTitle, { webkitTextStrokeWidth: "1px", duration: 0.5 }, "<")
    ...
    if (principisBand) {
      ...
      const tl = gsap.timeline({ scrollTrigger: { trigger: principisBand, start: "top 85%", once: true } });
      tl.to(principisBand, { scaleX: 1, duration: 0.6, ease: "expo.out" });
      principisNumbers.forEach((num, i) => {
        const at = 0.3 + i * 0.15;
        tl.to(num, { yPercent: 0, scale: 1, autoAlpha: 1, duration: 0.5, ease: "expo.out" }, at);
        if (principisTexts[i]) tl.to(principisTexts[i], { y: 0, autoAlpha: 1, duration: 0.4 }, at + 0.1);
      });
      ...
      gsap.matchMedia().add(DESKTOP_QUERY, () => {
        gsap.to(principisNumbers, {
          xPercent: -10,
          ease: "none",
          scrollTrigger: { trigger: principisBand, start: "top bottom", end: "bottom top", scrub: true },
        });
      });
    }
    ...
    if (statCols.length) {
      ...
      gsap
        ...
        .to(statCols, { scaleY: 1, duration: 0.6, stagger: 0.07, ease: "expo.out" })
        .to(statLabels.filter(Boolean), { yPercent: 0, duration: 0.4, stagger: 0.05, ease: "power2.out" }, "-=0.3")
        .add(() => {
    ...
    if (grups.length) {
      ...
      ScrollTrigger.batch(grups, {
        ...
        onEnter: (batch) => {
          ...
          gsap.to(batch, { y: 0, autoAlpha: 1, duration: 0.5, stagger: 0.07, ease: "power2.out", overwrite: "auto" });
          gsap.to(
            idx.map((i) => grupCircles[i]).filter(Boolean),
            { scale: 1, duration: 0.5, stagger: 0.07, ease: "expo.out", overwrite: "auto" },
          );
        },
      });
      ...
      grups.forEach((grup, i) => {
        ...
        const fillIn = () => {
          ...
          gsap.to(title, { webkitTextStrokeWidth: "0px", color: "var(--color-negro)", duration: 0.3, overwrite: "auto" });
        };
        const fillOut = () => {
          gsap.to(circle, { scale: 1, duration: 0.3, overwrite: "auto" });
          gsap.to(title, { webkitTextStrokeWidth: "1px", color: "transparent", duration: 0.3, overwrite: "auto" });
        };
